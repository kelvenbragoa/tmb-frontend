...
const schema = yup.object({
    ...
    username: yup.string().trim().label('Username'),
    role: yup.string().required().trim().label('Role'),
...
const onSubmit = handleSubmit((values) => {
    ...
    axios
        ...
        .then((response) => {
            // resetForm();
            router.back();
            toast.add({ severity: 'success', summary: `Successo`, detail: 'Usuário criado com sucesso', life: 3000 });
        })
        .catch((error) => {
            ...
            <div class="card p-fluid">
                ...
                <form @submit.prevent="onSubmit">
                    <div class="field">
                        ...
                        <InputText v-model="name" id="name" placeholder="Nome do Usuário" :class="{ 'p-invalid': errors.name }" type="text" />
                        <small id="name-help" class="p-error">{{ errors.name }}</small>
                    </div>
                    <div class="field">
                        <label for="username">Usuario</label>
                        <InputText v-model="username" id="username" placeholder="Usuario" :class="{ 'p-invalid': errors.username }" type="username" />
                        <small id="username-help" class="p-error">{{ errors.username }}</small>
                        <small>Se deixar em branco o sistema ira gerar um usuario automaticamente</small>
                    </div>
                    <div class="field">
                        <label for="role">Função *</label>
                        <Dropdown v-model="role" id="role" :options="roleOptions" optionLabel="label" optionValue="value" placeholder="Selecione uma função" :class="{ 'p-invalid': errors.role }" class="w-full" />
                        <small id="role-help" class="p-error">{{ errors.role }}</small>
                    </div>
                    <div class="field">
                        <label for="password">Palavra passe</label>
                        <InputText v-model="password" id="password" placeholder="Palavra passe" :class="{ 'p-invalid': errors.password }" type="password" />
    ...
    <!-- <div class="flex flex-col md:flex-row gap-12" v-else>
        <div class="w-full">
            ...
            <div class="card flex flex-col gap-4">
                ...
                <div class="font-semibold text-xl">Usuário</div>
                <small class="p-error">Os campos marcados * sao considerados campos obrigatorios.</small>
                <form @submit="onSubmit">
                    <div class="flex flex-col gap-2">
                        <label for="name1">Nome</label>
                        <InputText v-model="name" id="name" placeholder="Nome da Usuário" :class="{ 'p-invalid': errors.name }" type="text" />
                        <small id="name-help" class="p-error">{{ errors.name }}</small>
