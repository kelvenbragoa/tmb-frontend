...
const getData = async () => {
    ...
    try {
        ...
        formData.start_time = response.start_time;
        formData.end_time = response.end_time;
...
const submitForm = async () => {
    ...
    try {
        await ShiftService.updateShift(shiftId.value, formData);
        toast.add({ 
            severity: 'success', 
            summary: 'Sucesso', 
            detail: 'Turno atualizado com sucesso', 
            life: 3000 
        });
        router.push(`/transport/shifts/${shiftId.value}`);
    } catch (error) {
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao atualizar turno', 
            life: 3000 
        });
    } finally {
        loading.value = false;
