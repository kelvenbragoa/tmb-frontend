...
const getData = async (page = 1) => {
    loading.value = true;
    try {
        const response = await RouteCategoryService.getAllRouteCategories({
            page,
        ...
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao carregar categorias de rotas', 
            life: 3000 
        });
    } finally {
        loading.value = false;
...
const deleteData = async () => {
    ...
    try {
        await RouteCategoryService.deleteRouteCategory(dataIdBeingDeleted.value);
        ...
        <div class="col-12">
            <div class="card">
                <div class="flex align-items-center justify-content-between mb-4">
                    <h5 class="m-0">Categorias de Rotas</h5>
                    <router-link to="/transport/route-categories/create">
                ...
                >
                    ...
                    <Column header="Nome da Categoria" style="min-width: 15rem">
                        <template #body="{ data }">
                    ...
                    <Column header="Criado por" style="min-width: 12rem">
                        <template #body="{ data }">
                            <div class="font-medium">{{ data.createdBy?.name || '-' }}</div>
                        </template>
                    </Column>
                    ...
                    <Column header="Ações" style="min-width: 12rem">
                        <template #body="{ data }">
                            <div class="flex gap-2">
                                <router-link :to="'/transport/route-categories/' + data.id">
                                    <Button icon="pi pi-eye" class="p-button-text p-button-sm" v-tooltip.top="'Visualizar'" />
                                </router-link>
                                <router-link :to="'/transport/route-categories/' + data.id + '/edit'">
