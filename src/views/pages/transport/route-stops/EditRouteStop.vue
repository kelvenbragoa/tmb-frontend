...
const getData = async () => {
    ...
    try {
        ...
        formData.order = response.order;
        formData.routeId = response.routeId;
...
const submitForm = async () => {
    ...
    try {
        await RouteStopService.updateRouteStop(stopId.value, formData);
        toast.add({ 
            severity: 'success', 
            summary: 'Sucesso', 
            detail: 'Paragem atualizada com sucesso', 
            life: 3000 
        });
        // router.push(`/transport/route-stops/${stopId.value}`);
        goBack();
    } catch (error) {
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao atualizar paragem', 
            life: 3000 
        });
    } finally {
        loading.value = false;
        ...
        <div class="col-12">
            <div class="card">
                <div class="flex align-items-center justify-content-between mb-4">
                    <h5 class="m-0">Editar Paragem de Rota</h5>
