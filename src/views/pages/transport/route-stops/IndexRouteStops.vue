...
const getData = async (page = 1) => {
    ...
    try {
        ...
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao carregar paragens', 
            life: 3000 
        });
    } finally {
        loading.value = false;
...
const deleteData = async () => {
    ...
    try {
        await RouteStopService.deleteRouteStop(dataIdBeingDeleted.value);
        retrievedData.value = retrievedData.value.filter((data) => data.id !== dataIdBeingDeleted.value);
        closeConfirmation();
        toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Paragem excluída com sucesso', life: 3000 });
    } catch (error) {
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
        ...
        <div class="col-12">
            <div class="card">
                <div class="flex align-items-center justify-content-between mb-4">
                    <h5 class="m-0">Paragens de Rotas</h5>
                    <router-link to="/transport/route-stops/create">
                        <Button label="Nova Paragem" class="mr-2 mb-2"><i class="pi pi-plus"></i></Button>
                    </router-link>
                </div>
                ...
                >
                    <template #header>
                        <div class="flex justify-content-between flex-column sm:flex-row">
                            <IconField>
                                ...
                                <InputText v-model="searchQuery" placeholder="Pesquisar paragens..." />
                            </IconField>
                        </div>
                    </template>
                    <template #empty>Nenhuma paragem encontrada.</template>
                    ...
                    <!-- <Column header="Ordem" style="min-width: 6rem">
                        <template #body="{ data }">
                            <Tag :value="data.order" severity="info" />
                        </template>
                    </Column> -->

                    <Column header="Nome da Paragem" style="min-width: 15rem">
                        <template #body="{ data }">
                    ...
                    <Column header="Rota" style="min-width: 20rem">
                        <template #body="{ data }">
                            <div v-if="data.route">
