...
const getData = async (page = 1) => {
    ...
    try {
        ...
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao carregar rotas', 
            life: 3000 
        });
    } finally {
        loading.value = false;
...
const deleteData = async () => {
    ...
    try {
        await RouteService.deleteRoute(dataIdBeingDeleted.value);
        retrievedData.value = retrievedData.value.filter((data) => data.id !== dataIdBeingDeleted.value);
        closeConfirmation();
        toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Rota excluída com sucesso', life: 3000 });
    } catch (error) {
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao excluir rota', 
            life: 3000 
        });
    } finally {
...
<template>
        ...
        <div class="col-12">
            <div class="card">
                <div class="flex align-items-center justify-content-between mb-4">
                    <h5 class="m-0">Rotas de Transporte</h5>
                    <router-link to="/transport/routes/create">
                        <Button label="Nova Rota" class="mr-2 mb-2"><i class="pi pi-plus"></i></Button>
                    </router-link>
                </div>
                ...
                >
                    <template #header>
                        <div class="flex justify-content-between flex-column sm:flex-row">
                            ...
                            <IconField>
                                ...
                                <InputText v-model="searchQuery" placeholder="Pesquisar rotas..." />
                            </IconField>
                        </div>
                    </template>
                    <template #empty>Nenhuma rota encontrada.</template>
                    ...
                    <Column header="Nome da Rota" style="min-width: 15rem">
                        <template #body="{ data }">
                    ...
                    <Column header="Origem → Destino" style="min-width: 20rem">
                        <template #body="{ data }">
                    ...
                    <Column header="Categoria da Rota" style="min-width: 15rem">
                        <template #body="{ data }">
                            <div v-if="data.category" class="text-sm">
                                <div class="font-medium">{{ data.category.name ?? '-' }}</div>
                                <div class="text-sm text-600">{{ data.category.description || '-' }}</div>
                            </div>
                            
                        </template>
                    </Column>

                    <!-- <Column header="Informações Extras" style="min-width: 15rem">
                        <template #body="{ data }">
                            <div v-if="data.metadata">
                                <div v-if="data.metadata.distance" class="text-sm">
                                    <i class="pi pi-map-marker mr-1"></i>
                                    {{ data.metadata.distance }}
                                </div>
                                <div v-if="data.metadata.duration" class="text-sm text-600">
                    ...
                    <Column header="Ações" style="min-width: 12rem">
                        <template #body="{ data }">
                            <div class="flex gap-2">
                                <router-link :to="'/transport/routes/' + data.id">
                                    <Button icon="pi pi-eye" class="p-button-text p-button-sm" v-tooltip.top="'Visualizar'" />
                                </router-link>
                                <router-link :to="'/transport/routes/' + data.id + '/edit'">
    ...
    <Dialog header="Confirmação" v-model:visible="displayConfirmation" :style="{ width: '350px' }" :modal="true">
        <div class="flex align-items-center justify-content-center">
            ...
            <span>Tem certeza que deseja excluir esta rota?</span>
        </div>
        <template #footer>
