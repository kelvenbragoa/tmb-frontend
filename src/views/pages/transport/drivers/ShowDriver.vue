<script setup>
import DriverService from '@/service/DriverService';
import { ref, onMounted, computed } from 'vue';
import { useRouter, useRoute } from 'vue-router';
import { useToast } from 'primevue/usetoast';
import moment from 'moment';

const router = useRouter();
const route = useRoute();
const toast = useToast();
const loading = ref(false);
const driverData = ref(null);
const displayConfirmation = ref(false);
const loadingButtonDelete = ref(false);

const driverId = computed(() => parseInt(route.params.id));

const getData = async () => {
    loading.value = true;
    try {
        const response = await DriverService.getDriverById(driverId.value);
        driverData.value = response;
    } catch (error) {
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao carregar motorista', 
            life: 3000 
        });
        router.push('/transport/drivers');
    } finally {
        loading.value = false;
    }
};

const goBack = () => {
    router.back();
};

const confirmDeletion = () => {
    displayConfirmation.value = true;
};

const closeConfirmation = () => {
    displayConfirmation.value = false;
};

const deleteData = async () => {
    loadingButtonDelete.value = true;
    try {
        await DriverService.deleteDriver(driverId.value);
        toast.add({ 
            severity: 'success', 
            summary: 'Sucesso', 
            detail: 'Motorista excluído com sucesso', 
            life: 3000 
        });
        router.push('/transport/drivers');
    } catch (error) {
        toast.add({ 
            severity: 'error', 
            summary: 'Erro', 
            detail: error.response?.data?.message || 'Erro ao excluir motorista', 
            life: 3000 
        });
    } finally {
        loadingButtonDelete.value = false;
        closeConfirmation();
    }
};

onMounted(() => {
    getData();
});
</script>

<template>
    <div class="grid" v-if="loading">
        <div class="col-12 flex justify-content-center">
            <ProgressSpinner style="width: 50px; height: 50px"  animationDuration=".5s" aria-label="Custom ProgressSpinner" />
        </div>
    </div>
    
    <div class="grid" v-else-if="driverData">
        <div class="col-12">
            <div class="card">
                <div class="flex align-items-center justify-content-between mb-4">
                    <h5 class="m-0">Detalhes do Motorista</h5>
                    <div class="flex gap-2">
                        <Button label="Voltar" @click="goBack" severity="secondary">
                            <i class="pi pi-arrow-left"></i>
                        </Button>
                        <router-link :to="'/transport/drivers/' + driverId + '/edit'">
                            <Button label="Editar" severity="info">
                                <i class="pi pi-pencil"></i>
                            </Button>
                        </router-link>
                        <Button 
                            label="Excluir" 
                            severity="danger" 
                            @click="confirmDeletion"
                        >
                            <i class="pi pi-trash"></i>
                        </Button>
                    </div>
                </div>

                <div class="grid">
                    <!-- Informações Básicas -->
                    <div class="col-12 md:col-6">
                        <div class="card border-1 surface-border">
                            <h6 class="text-900 font-semibold mb-3">Informações Básicas</h6>
                            
                            <div class="field mb-3">
                                <label class="font-medium text-900">ID:</label>
                                <div class="mt-1">{{ driverData.id }}</div>
                            </div>

                            <div class="field mb-3">
                                <label class="font-medium text-900">Nome:</label>
                                <div class="mt-1 font-medium">{{ driverData.name }}</div>
                            </div>

                            <div class="field mb-0">
                                <label class="font-medium text-900">Status:</label>
                                <div class="mt-1">
                                    <Tag :value="driverData.is_active ? 'Ativo' : 'Inativo'" :severity="driverData.is_active ? 'success' : 'danger'" />
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Informações de Contacto -->
                    <div class="col-12 md:col-6">
                        <div class="card border-1 surface-border">
                            <h6 class="text-900 font-semibold mb-3">Contacto e Documentação</h6>
                            
                            <div class="field mb-3">
                                <label class="font-medium text-900">Telemóvel:</label>
                                <div class="mt-1">
                                    <div v-if="driverData.mobile" class="flex align-items-center gap-2">
                                        <i class="pi pi-phone text-600"></i>
                                        <span class="font-medium">{{ driverData.mobile }}</span>
                                    </div>
                                    <span v-else class="text-600">Não informado</span>
                                </div>
                            </div>

                            <div class="field mb-0">
                                <label class="font-medium text-900">Carta de condução:</label>
                                <div class="mt-1">
                                    <div v-if="driverData.document">
                                        <Tag :value="driverData.document" severity="secondary" />
                                    </div>
                                    <span v-else class="text-600">Não informado</span>
                                </div>
                            </div>

                            <div class="field mt-3 mb-0">
                                <label class="font-medium text-900">Data de Validade do Documento:</label>
                                <div class="mt-1">
                                    <div v-if="driverData.document_expiry_date">
                                        {{ moment(driverData.document_expiry_date).format('DD/MM/YYYY') }}
                                    </div>
                                    <span v-else class="text-600">Não informado</span>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- Informações de Auditoria -->
                    <div class="col-12">
                        <div class="card border-1 surface-border">
                            <h6 class="text-900 font-semibold mb-3">Informações de Auditoria</h6>
                            
                            <div class="grid">
                                <div class="col-12 md:col-6">
                                    <div class="field mb-3">
                                        <label class="font-medium text-900">Criado por:</label>
                                        <div class="mt-1">{{ driverData.createdBy?.name || '-' }}</div>
                                    </div>

                                    <div class="field mb-0">
                                        <label class="font-medium text-900">Data de Criação:</label>
                                        <div class="mt-1">{{ moment(driverData.createdAt).format('DD/MM/YYYY HH:mm:ss') }}</div>
                                    </div>
                                </div>

                                <div class="col-12 md:col-6">
                                    <div class="field mb-3">
                                        <label class="font-medium text-900">Atualizado por:</label>
                                        <div class="mt-1">{{ driverData.updatedBy?.name || '-' }}</div>
                                    </div>

                                    <div class="field mb-0">
                                        <label class="font-medium text-900">Última Atualização:</label>
                                        <div class="mt-1">{{ moment(driverData.updatedAt).format('DD/MM/YYYY HH:mm:ss') }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <Dialog header="Confirmação" v-model:visible="displayConfirmation" :style="{ width: '350px' }" :modal="true">
        <div class="flex align-items-center justify-content-center">
            <i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
            <span>Tem certeza que deseja excluir este motorista?</span>
        </div>
        <template #footer>
            <Button label="Cancelar" icon="pi pi-times" @click="closeConfirmation" class="p-button-text" />
            <Button label="Excluir" icon="pi pi-check" @click="deleteData" class="p-button-text" :loading="loadingButtonDelete" autofocus />
        </template>
    </Dialog>
</template>
