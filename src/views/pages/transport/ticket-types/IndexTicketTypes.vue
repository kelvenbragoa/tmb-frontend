...
const deleteData = async () => {
    ...
    try {
        await TicketTypeService.deleteTicketType(dataIdBeingDeleted.value);
        ...
        <div class="col-12">
            <div class="card">
                <div class="flex align-items-center justify-content-between mb-4">
                    <h5 class="m-0">Tipos de Bilhete</h5>
                    <router-link to="/transport/ticket-types/create">
                        <Button label="Novo Tipo de Bilhete" class="mr-2 mb-2"><i class="pi pi-plus"></i></Button>
                    </router-link>
                </div>
                ...
                >
                    <template #header>
                        <div class="flex justify-content-between flex-column sm:flex-row">
                            <!-- <Button label="Voltar" @click="goBackUsingBack" class="mr-2 mb-2" severity="secondary">
                            ...
                            <IconField>
                                ...
                                <InputText v-model="searchQuery" placeholder="Pesquisar tipos de bilhete..." />
                            </IconField>
                        </div>
                    </template>
                    <template #empty>Nenhum tipo de bilhete encontrado.</template>
                    ...
                    <Column header="Código" style="min-width: 10rem">
                        <template #body="{ data }">
                    ...
                    <Column header="Data de Criação" dataType="date" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ moment(data.createdAt).format('DD/MM/YYYY HH:mm') }}
                        </template>
                    </Column>

                    <Column header="Criado por" style="min-width: 12rem">
                        <template #body="{ data }">
                            {{ data.createdBy?.name || '-' }}
                        </template>
                    </Column>
                    ...
                    <Column header="Ações" style="min-width: 12rem">
                        <template #body="{ data }">
                            <div class="flex gap-2">
                                <router-link :to="'/transport/ticket-types/' + data.id">
                                    <Button icon="pi pi-eye" class="p-button-text p-button-sm" v-tooltip.top="'Visualizar'" />
                                </router-link>
                                <router-link :to="'/transport/ticket-types/' + data.id + '/edit'">
