...
const getData = async (page = 1) => {
    ...
    setTimeout(() => {
        const mockData = [
            { id: 1, name: 'Dinheiro', code: 'CASH', type: 'ESPECIE', status: true, createdAt: new Date('2024-01-01') },
            { id: 2, name: 'Transferência Bancária', code: 'BANK', type: 'ELETRONICO', status: true, createdAt: new Date('2024-01-01') },
            { id: 3, name: 'M-Pesa', code: 'MPESA', type: 'MOBILE', status: true, createdAt: new Date('2024-01-02') },
            { id: 4, name: 'E-Mola', code: 'EMOLA', type: 'MOBILE', status: true, createdAt: new Date('2024-01-02') },
            { id: 5, name: 'Cartão de Crédito', code: 'CC', type: 'CARTAO', status: true, createdAt: new Date('2024-01-03') },
            { id: 6, name: 'Cheque', code: 'CHQ', type: 'CHEQUE', status: false, createdAt: new Date('2024-01-03') }
        ];
...
const deleteData = () => {
    ...
    axios
        .delete(`${baseURL}/settings/payment-methods/${dataIdBeingDeleted.value}`)
        .then(() => {
            retriviedData.value = retriviedData.value.filter((data) => data.id !== dataIdBeingDeleted.value);
            displayConfirmation.value = false;
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Método de pagamento apagado com sucesso', life: 3000 });
        })
        .catch((error) => {
...
<template>
    ...
    <div class="grid" v-else>
        <div class="col-12">
            <div class="card">
                <h5>Gestão de Métodos de Pagamento</h5>
                <DataTable
                    :value="retriviedData"
                    :paginator="true"
                    :rows="rowsPerPage"
                ...
                >
                    <template #header>
                        <div class="flex justify-content-between flex-column sm:flex-row">
                            <router-link to="/settings/payment-methods/create">
                                <Button label="Novo Método de Pagamento" class="mr-2 mb-2"><i class="pi pi-plus"></i></Button>
                            </router-link>
                            <IconField>
                                <InputIcon>
                                    <i class="pi pi-search" />
                                </InputIcon>
                                <InputText v-model="searchQuery" placeholder="Pesquisar métodos de pagamento" />
                            </IconField>
                        </div>
                    </template>
                    <template #empty>Nenhum método de pagamento encontrado.</template>
                    ...
                    <Column field="type" header="Tipo" style="min-width: 10rem">
                        <template #body="{ data }">
                            <Tag :value="data.type" severity="warning"></Tag>
                        </template>
                    </Column>
                    ...
                    <Column header="Ações" style="min-width: 12rem">
                        <template #body="{ data }">
                            <router-link class="m-3" :to="'/settings/payment-methods/' + data.id + '/edit'">
                                <i class="pi pi-file-edit text-primary"></i>
                            </router-link>
                            <router-link class="m-3" :to="'/settings/payment-methods/' + data.id">
    ...
    <Dialog header="Confirmação" v-model:visible="displayConfirmation" :style="{ width: '350px' }" :modal="true">
        <div class="flex align-items-center justify-content-center">
            ...
            <span>Tem certeza que deseja apagar este método de pagamento?</span>
        </div>
        <template #footer>
