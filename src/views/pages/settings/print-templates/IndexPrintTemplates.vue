...
const getData = async (page = 1) => {
    axios.get(`${baseURL}/settings/print-templates?page=${page}`, { params: { query: searchQuery.value } })
        .then((response) => {
...
const deleteData = () => {
    ...
    axios.delete(`${baseURL}/settings/print-templates/${dataIdBeingDeleted.value}`)
        .then(() => {
            retriviedData.value = retriviedData.value.filter((data) => data.id !== dataIdBeingDeleted.value);
            displayConfirmation.value = false;
            toast.add({ severity: 'success', summary: 'Sucesso', detail: 'Modelo de impressão apagado com sucesso', life: 3000 });
        })
        .catch((error) => {
...
<template>
    ...
    <div class="grid" v-else>
        <div class="col-12">
            <div class="card">
                <h5>Gestão de Modelos de Impressão</h5>
                <DataTable :value="retriviedData" :paginator="true" :rows="rowsPerPage" :totalRecords="totalRecords" dataKey="id" :lazy="true" :rowHover="true" :loading="isLoadingDiv" :first="(currentPage - 1) * rowsPerPage" :onPage="onPageChange" showGridlines>
                    <template #header>
                        <div class="flex justify-content-between flex-column sm:flex-row">
                            <router-link to="/settings/print-templates/create">
                                <Button label="Novo Modelo de Impressão" class="mr-2 mb-2"><i class="pi pi-plus"></i></Button>
                            </router-link>
                            <IconField>
                                <InputIcon><i class="pi pi-search" /></InputIcon>
                                <InputText v-model="searchQuery" placeholder="Pesquisar modelos de impressão" />
                            </IconField>
                        </div>
                    </template>
                    <template #empty>Nenhum modelo de impressão encontrado.</template>
                    ...
                    <Column field="format" header="Formato" style="min-width: 8rem">
                        <template #body="{ data }">
                            <Tag :value="data.format" severity="warning"></Tag>
                        </template>
                    </Column>
                    ...
                    <Column header="Ações" style="min-width: 12rem">
                        <template #body="{ data }">
                            <router-link class="m-3" :to="'/settings/print-templates/' + data.id + '/edit'">
                                <i class="pi pi-file-edit text-primary"></i>
                            </router-link>
                            <router-link class="m-3" :to="'/settings/print-templates/' + data.id">
    ...
    <Dialog header="Confirmação" v-model:visible="displayConfirmation" :style="{ width: '350px' }" :modal="true">
        <div class="flex align-items-center justify-content-center">
            ...
            <span>Tem certeza que deseja apagar este modelo de impressão?</span>
        </div>
        <template #footer>
