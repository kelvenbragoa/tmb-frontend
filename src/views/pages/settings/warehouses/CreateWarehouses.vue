...
const schema = yup.object({
    ...
    // is_main_stock_center: yup.required().trim().label('Description'),
    // location: yup.string().required().trim().label('Location'),
...
const [is_main_stock_center] = defineField('is_main_stock_center');
...
const onSubmit = handleSubmit((values) => {
    ...
    axios
        .post(`${baseURL}/stockcenters`, values)
        .then((response) => {
            // resetForm();
            router.back();
            toast.add({ severity: 'success', summary: `Successo`, detail: 'Ponto de Venda criado com sucesso', life: 3000 });
        })
        .catch((error) => {
...
const getData = async (page = 1) => {
    axios
        .get(`${baseURL}/api/stockcenters/create`, {
            params: {
...
const deleteData = () => {
    ...
    axios
        .delete(`/api/stockcenters/${dataIdBeingDeleted.value}`)
        .then(() => {
            ...
            <div class="card p-fluid">
                ...
                <form @submit.prevent="onSubmit">
                    <div class="field">
                        ...
                        <InputText v-model="name" id="name" placeholder="Nome Armazem" :class="{ 'p-invalid': errors.name }" type="text" />
                        <small id="name-help" class="p-error">{{ errors.name }}</small>
                    </div>
                    
                    <div class="field">
                        <label for="is_main_stock_center">É Armazém Principal?</label>
                        <Checkbox v-model="is_main_stock_center" id="is_main_stock_center" :binary="true" :true-value="1" :false-value="0" />
                    </div>
